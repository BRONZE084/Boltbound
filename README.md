...
**界面、动画与平台：人物和场景动画、操作反馈与多端适配。** 负责人物与场景的美术和动画设计及视觉实现，同时让玩家清楚地选择、搭建和使用道具，在浏览器、手机及微信小游戏中获得一致的操作体验与反馈。

### 开发重点

- 设计并实现人物动作、场景动态、机关表现和道具特效，统一游戏的视觉风格与动画节奏。
...
- **人物动画**：人物造型与动作设计，以及待机、移动、起跳、空中、落地、受击、死亡、复位和使用道具等状态的动画与衔接。
- **场景与机关动画**：背景环境动态、移动平台、机关运转、危险预警、触发、爆破、传送和状态结束等视觉表现，保证关键危险提示清晰可辨。
- **道具表现**：设计增益、减速、重力、反向控制、护盾、烟雾等效果的视觉提示，处理多个状态同时存在时的辨识度。
- **跨端实现与交付**：维护素材、资源清单和共用场景中的动画表现，提供动作状态表、触发时机说明、预览图或录屏；验证浏览器与微信端的显示效果和性能。
...
| [jima/public/assets/](jima/public/assets/) | 人物、场景、机关与道具的美术和动画源素材 |
| [jima/shared/gameAssets.js](jima/shared/gameAssets.js) | 两端共用的资源清单、尺寸和纹理映射 |
| [jima/src/game/BoltboundScene.js](jima/src/game/BoltboundScene.js) | 共用场景中的人物与机关动画、特效接入（与玩法分支协作） |

### 协作边界与交付

本分支主责人物与场景动画的美术、动作设计和视觉实现；玩法分支负责运动轨迹、碰撞、伤害、道具效果和触发条件。预警、命中、死亡等动画时机以玩法状态为依据，调整动画长度时共同核对有效判定时段。修改共用的 BoltboundScene.js 或素材尺寸前，先对齐修改位置、资源尺寸与碰撞体的对应关系。

涉及远端角色的动作状态和动画触发事件时，与联机分支对齐事件、方向和时间信息。共用场景动画由浏览器、微信版本及道具实验室复用；实验室专用界面由玩法分支维护。

提供界面与动画变化说明、动作状态和触发时机、素材尺寸及适用设备范围。验证动作衔接、预警和命中反馈、不同帧率下的播放效果，以及键鼠、触控、横竖屏和相关设置与 HUD 测试；记录浏览器和微信端各自的验证结果。
