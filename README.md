...
从游戏首页点击「道具实验室」，或在 `jima/` 执行 `npm run dev:lab` 后打开 `http://localhost:5173/lab.html`。实验室直接读取源码中的 14 种搭建零件与 9 种主动道具，复用原版游戏场景，支持自由搭建、道具试用、设置出生点、撤销重做和方案导入导出，不需要房间服务器。
...
- 实验台「搭建」类的横梁、方箱、移动路障和冰面之间允许贴边叠放、拼接与旋转接合，放置时实体不能重叠。移动路障在独立运动层运行，可穿过其他零件，显示在固定零件后方，仍与角色碰撞；静态零件不会随其移动。
- 上轮死亡角色不再阻挡建造；隐藏的尸体不应继续占据放置位置。
- 移动路障的整段轨迹受地图边界、保护区与当前角色限制，但可与其他零件相交；传送门检查出口，部分机关还检查风力、射线或弹射作用范围，而不只是图标外框。
