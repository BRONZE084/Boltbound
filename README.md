...
**分支：`feature/gameplay-traps`**

**核心玩法：搭建、道具与陷阱交互。** 负责玩家能搭什么、道具怎样使用，以及人物接触场景和机关后会发生什么；为游戏的核心交互体验提供实现与调试工具。

### 开发重点

- 完善零件放置、旋转、碰撞与建造限制，设计可组合的陷阱和机关。
- 优化人物移动、跳跃、弹射、传送、风力、冰面等场景交互，以及主动道具的增益、干扰和爆破效果。
- 维护道具实验室，用可保存和交换的搭建方案复现交互问题，验证机关组合与道具作用。
...
每次优化改动完成后，在[分支优化日志](CHANGELOG.md)中记录优化对象、修改前后的行为、涉及文件和实际验证结果，并与对应代码一同提交；未验证的内容应明确标注。

优化日志、代码变更注释、对应测试说明和后续 Git 提交说明统一使用中文；源码标识符、路径、命令与错误码保留原文。
...
从游戏首页点击「道具实验室」，或在 `jima/` 执行 `npm run dev:lab` 后打开 `http://localhost:5173/lab.html`。实验室直接读取源码中的 16 种搭建零件与 9 种主动道具，复用原版游戏场景，支持自由搭建、道具试用、设置出生点、撤销重做和方案导入导出，不需要房间服务器。

试用不限时、不计分，重点观察人物、场景、机关和道具的交互。详细操作与源码边界见 [道具实验室说明](jima/docs/TRAP_LAB.md)。
...
出生基点为 `(80, 142)`，多人按槽位错开。终点区域中心为 `(1510, 595)`。配置来源为 [shared/gameConfig.js](jima/shared/gameConfig.js)；其中 `groundY` 是旧地图参考高度，不代表实心地板，也不再限制搭建范围。
...
- 全画面允许搭建，零件中心范围为 `0 <= x <= 1600`、`0 <= y <= 900`。边缘允许零件部分伸出画面；客户端预览、实验台精确输入与服务端采用同一范围。
- 出生区不预留禁建空间；仅保护旗子碰撞体 `X=1465–1555、Y=530–660`，零件实体及运动轨迹不得侵入；不额外保留通道，边缘相接可放置。当前人物、已有零件和实际传送出口仍检查占用冲突。
- 实验台「搭建」类的横梁、方箱、移动路障和冰面之间允许贴边叠放、拼接与旋转接合，放置时实体不能重叠。移动路障在独立运动层运行，可穿过其他零件，显示在固定零件上方、人物下方，仍与角色碰撞；静态零件不会随其移动。
- 上轮死亡角色不再阻挡建造；隐藏的尸体不应继续占据放置位置。
- 除旗子碰撞体保护区外，移动轨迹、风力、射线和弹射作用范围不占用搭建位置。路障运行行程按画面边界缩短；传送出口按人物尺寸收回画面内，并检查实体冲突。
- 黑洞只按实体与其他物体、人物检查占用，吸引范围可以伸出画面。
- 与基础平台的重叠规则按零件类型区分：横梁、方箱、弹簧、尖刺与其余机关不同，不能简单理解为所有图形重叠都禁止。

遇到红色预览时，检查零件中心、实体占用和传送出口。人物站上移动路障后按其实际位移运动；无阻挡时保持相对位置，可正常行走或跳离。碰侧墙时人物停在墙前，平台继续从脚下移动；碰顶或承托平台被移除后解除承托。

## 机关与主动道具

选件池共有 25 种选项：16 种地图零件、9 种竞速主动道具。名称和参数集中在 [shared/gameConfig.js](jima/shared/gameConfig.js)，以下为当前默认值。
...
| `windmill` | 风车平台 | `360 x 360` | 四个 `120 x 24` 的水平平台绕轴转动，半径 120，每圈 8 秒；平台之间及轮辐为空隙 |
| `rotatingCrate` | 旋转方箱 | `80 x 80` | 绕自身中心每 6 秒转一圈，每 90° 停顿 0.35 秒并平滑启停；按实际箱面碰撞与承托 |
...
构建会生成并同步音频，将 SVG 按需转换为 PNG，校验尺寸和透明背景，并输出单个游戏 bundle。当前资源清单包含 31 张 PNG 和 14 个 WAV；素材调整时以共享清单与构建校验为准。
...
除绿色旗子碰撞体保护区外，全画面均可搭建，但实体占用和传送出口仍可能冲突。零件中心须在画面内；活着的角色和已有机关仍可能阻挡，上轮尸体不阻挡。先旋转或移动一格；详细规则见[地图与放置规则](#地图与放置规则)。
